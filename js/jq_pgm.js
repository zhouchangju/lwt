...
function changeImprAnnText() {
  ...
  $.post(
    ...
    { id: textid, elem: elem, data: thedata },
    function (d) {
...
function do_ajax_update_media_select() {
  ...
  $.post('ajax_update_media_select.php', function (data) {
    $('#mediaselect').html(data);
  });
}
...
function do_ajax_word_counts() {
  $("span[id^='saved-']").each(function (i) {
    ...
    $.post('ajax_word_counts.php', { id: textid }, function (data) {
      var res = eval('(' + data + ')');
