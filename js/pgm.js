...
function run_overlib_status_1_to_5(
  ...
  return overlib(
    '<b>' +
      escape_html_chars_2(hints, ann) +
      '</b><br /> ' +
      make_overlib_link_change_status_all(txid, torder, wid, stat) +
      ' <br /> ' +
...
function run_overlib_status_unknown(
  ...
  return overlib(
    '<b>' +
      escape_html_chars(hints) +
      '</b><br /> ' +
      make_overlib_link_ignore_word(txid, torder) +
      ' <br /> ' +
      make_overlib_link_wellknown_word(txid, torder) +
      make_overlib_link_new_multiword(
        txid,
        torder,
        mw2,
...
function run_overlib_multiword(
  ...
  return overlib(
    '<b>' +
      ...
      ' | ' +
      make_overlib_link_delete_multiword(txid, wid) +
      ' <br /> ' +
      make_overlib_link_wb(wblink1, wblink2, wblink3, txt, txid, torder),
    CAPTION,
    make_overlib_link_edit_multiword_title(
      wcnt.trim() + '-Word-Expression',
      txid,
      torder,
      wid
    )
...
function make_overlib_link_new_multiword(
  ...
  if (rtl)
    return (
      ' <br />Expr: ' +
      (mw9 != ''
        ? make_overlib_link_create_edit_multiword_rtl(9, txid, torder, mw9) +
          ' '
        : '') +
      (mw8 != ''
        ? make_overlib_link_create_edit_multiword_rtl(8, txid, torder, mw8) +
          ' '
        : '') +
      (mw7 != ''
        ? make_overlib_link_create_edit_multiword_rtl(7, txid, torder, mw7) +
          ' '
        : '') +
      (mw6 != ''
        ? make_overlib_link_create_edit_multiword_rtl(6, txid, torder, mw6) +
          ' '
        : '') +
      (mw5 != ''
        ? make_overlib_link_create_edit_multiword_rtl(5, txid, torder, mw5) +
          ' '
        : '') +
      (mw4 != ''
        ? make_overlib_link_create_edit_multiword_rtl(4, txid, torder, mw4) +
          ' '
        : '') +
      (mw3 != ''
        ? make_overlib_link_create_edit_multiword_rtl(3, txid, torder, mw3) +
          ' '
        : '') +
      (mw2 != ''
        ? make_overlib_link_create_edit_multiword_rtl(2, txid, torder, mw2)
        : '') +
      ' '
    );
  else
    return (
      ' <br />Expr: ' +
      (mw2 != ''
        ? make_overlib_link_create_edit_multiword(2, txid, torder, mw2) + ' '
        : '') +
      (mw3 != ''
        ? make_overlib_link_create_edit_multiword(3, txid, torder, mw3) + ' '
        : '') +
      (mw4 != ''
        ? make_overlib_link_create_edit_multiword(4, txid, torder, mw4) + ' '
        : '') +
      (mw5 != ''
        ? make_overlib_link_create_edit_multiword(5, txid, torder, mw5) + ' '
        : '') +
      (mw6 != ''
        ? make_overlib_link_create_edit_multiword(6, txid, torder, mw6) + ' '
        : '') +
      (mw7 != ''
        ? make_overlib_link_create_edit_multiword(7, txid, torder, mw7) + ' '
        : '') +
      (mw8 != ''
        ? make_overlib_link_create_edit_multiword(8, txid, torder, mw8) + ' '
        : '') +
      (mw9 != ''
        ? make_overlib_link_create_edit_multiword(9, txid, torder, mw9)
        : '') +
      ' '
    );
}
...
function make_overlib_link_wbnl(wblink1, wblink2, wblink3, txt, txid, torder) {
  var s =
    ...
    (torder < 1 || txid < 1
      ? ''
      : ' | Sentence: ' + createSentLookupLink(torder, txid, wblink3, 'GTr'));
  return s;
}
...
function make_overlib_link_change_status_all(txid, torder, wid, oldstat) {
  ...
  for (var newstat = 1; newstat <= 5; newstat++)
    result += make_overlib_link_change_status(
      txid,
      torder,
      wid,
      oldstat,
...
function make_overlib_link_change_status(txid, torder, wid, oldstat, newstat) {
  if (oldstat == newstat) {
    return '<span title=\x22' + getStatusName(oldstat) + '\x22>◆</span>';
  } else {
    return (
      ' <a href=\x22set_word_status.php?tid=' +
      txid +
      '&amp;ord=' +
      torder +
      '&amp;wid=' +
...
function make_overlib_link_new_word(txid, torder, wid) {
  return (
    ' <a href=\x22edit_word.php?tid=' +
    txid +
    '&amp;ord=' +
    torder +
    '&amp;wid=' +
    wid +
    '\x22 target=\x22ro\x22>Learn term</a> '
  );
}

function make_overlib_link_edit_multiword(txid, torder, wid) {
  return (
    ' <a href=\x22edit_mword.php?tid=' +
    txid +
    '&amp;ord=' +
    torder +
    '&amp;wid=' +
    wid +
    '\x22 target=\x22ro\x22>Edit term</a> '
  );
}

function make_overlib_link_edit_multiword_title(text, txid, torder, wid) {
  return (
    '<a style=\x22color:yellow\x22 href=\x22edit_mword.php?tid=' +
    txid +
    '&amp;ord=' +
    torder +
    '&amp;wid=' +
...
function make_overlib_link_create_edit_multiword(len, txid, torder, txt) {
  return (
    ...
    len +
    '..' +
    escape_html_chars(txt.substr(-2).trim()) +
    '</a> '
  );
}
...
function make_overlib_link_create_edit_multiword_rtl(len, txid, torder, txt) {
  return (
    ' <a dir=\x22rtl\x22 href=\x22edit_mword.php?tid=' +
    txid +
    '&amp;ord=' +
    torder +
    '&amp;txt=' +
...
function make_overlib_link_edit_word_title(text, txid, torder, wid) {
  return (
    '<a style=\x22color:yellow\x22 href=\x22edit_word.php?tid=' +
    txid +
    '&amp;ord=' +
    torder +
    '&amp;wid=' +
...
function make_overlib_link_delete_word(txid, wid) {
  return (
    ' <a onclick=\x22return confirmDelete();\x22 href=\x22delete_word.php?wid=' +
    wid +
    '&amp;tid=' +
    txid +
    '\x22 target=\x22ro\x22>Delete term</a> '
  );
}

function make_overlib_link_delete_multiword(txid, wid) {
  return (
    ' <a onclick=\x22return confirmDelete();\x22 href=\x22delete_mword.php?wid=' +
    wid +
    '&amp;tid=' +
    txid +
    '\x22 target=\x22ro\x22>Delete term</a> '
...
function make_overlib_link_wellknown_word(txid, torder) {
  return (
    ' <a href=\x22insert_word_wellknown.php?tid=' +
    txid +
    '&amp;ord=' +
    torder +
    '\x22 target=\x22ro\x22>I know this term well</a> '
  );
}

function make_overlib_link_ignore_word(txid, torder) {
  return (
    ' <a style="font-size:35px"  href=\x22insert_word_ignore.php?tid=' +
    txid +
    '&amp;ord=' +
    torder +
    '\x22 target=\x22ro\x22>Ignore this term</a> '
  );
}
...
function escape_html_chars_2(title, ann) {
  if (ann != '') {
    ...
    return escape_html_chars(title).replace(
      ann2,
      '<span style="color:red">' + ann2 + '</span>'
    );
  } else return escape_html_chars(title);
}
...
function createSentLookupLink(torder, txid, url, txt) {
  ...
  if (url != '' && txt != '') {
    if (url.substr(0, 8) == '*http://' || url.substr(0, 9) == '*https://') {
      r =
        " <span class=\x22click\x22 onclick=\x22owin('trans.php?x=1&i=" +
        torder +
        '&t=' +
        txid +
        "');\x22>" +
